...
const JobSeeker = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch("http://localhost:3000/api/admin/jobseekers", {
        ...
        if (alive && json.success) {
          ...
          setError(json.message || "Failed to load job seekers");
        }
      } catch {
        if (alive) {
  ...
  const graphData = useMemo(() => {
    ...
    if (period === "year") {
      ...
      items.forEach(u => {
        if (!u.createdAt) return;
        const iso = new Date(u.createdAt).toISOString().split("T")[0];
    ...
    <div className="flex flex-col h-screen gap-12 px-6 py-8 overflow-auto">
      <div className="flex items-center justify-between mb-3">
        <h1 className="text-xl font-semibold">
          Job Seekers <span className="font-normal text-gray-500">{countText}</span>
        </h1>
      </div>
      ...
      <div className="h-[350px]">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">Job Seekers Joined</h2>
          <select
            value={period}
      ...
      <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
        {loading ? (
          ...
          <div className="text-sm text-gray-500">No job seekers found.</div>
        ) : (
          currentItems.map((u) => (
            <div
              key={u.id}
              ...
              className={`p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition cursor-pointer ${
                expandedId === u.id ? "" : "max-h-16 overflow-hidden"
              }`}
            >
              <div className="flex items-center justify-between mb-2">
                <div>
                  <div className="text-base text-gray-800">{u.name}</div>
              ...
              {expandedId === u.id && (
                <div className="text-sm text-gray-700">
                  {u.skills && <div>Skills: {Array.isArray(u.skills) ? u.skills.join(", ") : u.skills}</div>}
                  {u.createdAt && (
